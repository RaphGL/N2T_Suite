...
namespace {
   ...
   bool parse_args_to_map(
       std::unordered_map<const char *, uint16_t> &internal_pins, std::vector<Arg> args) {
      bool success { true };
   ...
   std::optional<std::unordered_map<const char *, uint16_t>> builtin_nand(
       PinMap &internal_pins, std::vector<Arg> args) {
   ...
   using BuiltinFunction = std::function<std::optional<std::unordered_map<const char *, uint16_t>>(
       PinMap &, std::vector<Arg>)>;

   std::unordered_map<const char *, BuiltinFunction> builtin_chips { { "Nand", builtin_nand } };
}; // namespace
